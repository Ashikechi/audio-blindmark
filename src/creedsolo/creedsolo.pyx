...
::
    ...
    Then to encode:
    >> mesecc = rs.rs_encode_msg(iter(mes), nsym, gen = gen[nsym])
    ...
    >> mesecc = rs.rs_encode_msg(iter(mes), nsym)
...
import array as pa
from typing import Optional, Iterable, Sequence, Generator, Annotated
...
cdef ca.array gf_exp = ca.clone(uint64_array_template, <Py_ssize_t>512, <bint>False) # For efficiency, gf_exp[] has size 2*GF_SIZE, so that a simple multiplication of two numbers can be resolved without calling % 255. For more infos on how to generate this extended exponentiation table, see paper: "Fast software implementation of finite field operations", Cheng Huang and Lihao Xu, Washington University in St. Louis, Tech. Rep (2003).
cdef ca.array gf_log = ca.clone(uint64_array_template, <Py_ssize_t>256, <bint>False)

cdef uint64_t field_charac = (<uint64_t>1 << 8) - 1
...
cpdef ca.array rwh_primes1(uint64_t n):
    ...
    cdef ca.array is_prime = ca.clone(uint8_array_template, <Py_ssize_t>n, <bint>False)
    cdef uint64_t i
    for i in range(2, <uint64_t>len(is_prime)): # pyright: ignore[reportGeneralTypeIssues]
        is_prime.data.as_uchars[i] = <uint8_t>1 # pyright: ignore[reportGeneralTypeIssues]
    is_prime.data.as_uchars[0] = is_prime.data.as_uchars[1] = <uint8_t>0 # pyright: ignore[reportGeneralTypeIssues]

    cdef ca.array primes = ca.clone(uint64_array_template, <Py_ssize_t>n, <bint>False)
    cdef uint64_t primes_size = <uint64_t>0
    cdef uint64_t j
    for i in range(2, n): # pyright: ignore[reportGeneralTypeIssues]
        if is_prime.data.as_uchars[i]: # pyright: ignore[reportGeneralTypeIssues]
            primes.data.as_ulonglongs[primes_size] = i # pyright: ignore[reportGeneralTypeIssues]
            primes_size += <uint64_t>1
        for j in range(<uint64_t>len(primes)): # pyright: ignore[reportGeneralTypeIssues]
            if i * primes.data.as_ulonglongs[j] >= n: # pyright: ignore[reportGeneralTypeIssues]
                break
            is_prime.data.as_uchars[i * primes.data.as_ulonglongs[j]] = <uint8_t>0 # pyright: ignore[reportGeneralTypeIssues]
            if i % primes.data.as_ulonglongs[j] == 0: # pyright: ignore[reportGeneralTypeIssues]
                break
    ca.resize(primes, <Py_ssize_t>primes_size)
    return primes

cpdef ca.array find_prime_polys(uint64_t generator = <uint64_t>2, uint64_t c_exp = <uint64_t>8, bool fast_primes = False, bool single = False):
    ...
    cdef uint64_t prim_candidates_size = <uint64_t>0
    ...
    if fast_primes:
        ...
        prim_candidates = ca.clone(uint64_array_template, <Py_ssize_t>len(primes), <bint>False) # pyright: ignore[reportGeneralTypeIssues]
        for i in range(<uint64_t>len(primes)): # pyright: ignore[reportGeneralTypeIssues] filter out too small primes
            if primes.data.as_ulonglongs[i] > field_charac: # pyright: ignore[reportGeneralTypeIssues]
                prim_candidates.data.as_ulonglongs[prim_candidates_size] = primes.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues]
                prim_candidates_size += <uint64_t>1
    else:
        prim_candidates = ca.clone(uint64_array_template, (<Py_ssize_t>1 << c_exp - 1) - 1, <bint>False)
        for i in range(field_charac + 2, field_charac_next, 2): # pyright: ignore[reportGeneralTypeIssues] try each possible prime polynomial, but skip even numbers (because divisible by 2 so necessarily not irreducible)
            prim_candidates.data.as_ulonglongs[prim_candidates_size] = i # pyright: ignore[reportGeneralTypeIssues]
            prim_candidates_size += 1
    ca.resize(prim_candidates, <Py_ssize_t>prim_candidates_size)

    # Start of the main loop
    cdef ca.array correct_primes = ca.clone(uint64_array_template, <Py_ssize_t>len(prim_candidates), <bint>False) # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t correct_primes_size = <uint64_t>0
    cdef ca.array seen
    cdef bool conflict
    cdef uint64_t _, x
    for i in range(<uint64_t>len(prim_candidates)): # pyright: ignore[reportGeneralTypeIssues] try potential candidates primitive irreducible polys
        seen = ca.clone(uint64_array_template, <Py_ssize_t>(field_charac + 1), <bint>True) # memory variable to indicate if a value was already generated in the field (value at index x is set to 1) or not (set to 0 by default)
        conflict = False # flag to know if there was at least one conflict

        # Second loop, build the whole Galois Field
        x = <uint64_t>1
        for _ in range(field_charac): # pyright: ignore[reportGeneralTypeIssues]
            # Compute the next value in the field (ie, the next power of alpha/generator)
            x = gf_mult_noLUT(x, generator, prim_candidates.data.as_ulonglongs[i], field_charac + 1) # pyright: ignore[reportGeneralTypeIssues]
            ...
            if x > field_charac or seen.data.as_ulonglongs[x] == 1: # pyright: ignore[reportGeneralTypeIssues]
                conflict = True
                break
            # Else we flag this value as seen (to maybe detect future duplicates), and we continue onto the next power of alpha
            else:
                seen.data.as_ulonglongs[x] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]

        # End of the second loop: if there's no conflict (no overflow nor duplicated value), this is a prime polynomial!
        if not conflict:
            if single:
                ca.resize(correct_primes, <Py_ssize_t>1)
                correct_primes.data.as_ulonglongs[0] = prim_candidates.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues]
                return correct_primes # for API consistency, we always return an array, but here with a single value
            correct_primes.data.as_ulonglongs[correct_primes_size] = prim_candidates.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues]
            correct_primes_size += 1

    ca.resize(correct_primes, <Py_ssize_t>correct_primes_size)
...
cpdef tuple init_tables(uint64_t prim = <uint64_t>0x11d, uint64_t generator = <uint64_t>2, uint64_t c_exp = <uint64_t>8):
    ...
    field_charac = (1 << c_exp) - 1
    gf_exp = ca.clone(uint64_array_template, <Py_ssize_t>(field_charac * 2), <bint>False) # anti-log (exponential) table. The first two elements will always be [GF256int(1), generator]
    gf_log = ca.clone(uint64_array_template, <Py_ssize_t>(field_charac + 1), <bint>False) # log table, log[0] is impossible and thus unused
    ...
    cdef uint64_t x = <uint64_t>1
    cdef uint64_t i
    for i in range(field_charac): # pyright: ignore[reportGeneralTypeIssues] we could skip index 255 which is equal to index 0 because of modulo: g^255==g^0 but either way, this does not change the later outputs (ie, the ecc symbols will be the same either way)
        gf_exp.data.as_ulonglongs[i] = x # pyright: ignore[reportGeneralTypeIssues] compute anti-log for this value and store it in a table
        gf_log.data.as_ulonglongs[x] = i # pyright: ignore[reportGeneralTypeIssues] compute log at the same time
    ...
    for i in range(field_charac, field_charac * 2): # pyright: ignore[reportGeneralTypeIssues]
        gf_exp.data.as_ulonglongs[i] = gf_exp.data.as_ulonglongs[i - field_charac] # pyright: ignore[reportGeneralTypeIssues]
...
cpdef uint64_t gf_inverse(uint64_t x):
    ...
    return gf_exp.data.as_ulonglongs[field_charac - gf_log.data.as_ulonglongs[x]] # pyright: ignore[reportGeneralTypeIssues] gf_inverse(x) == gf_div(1, x)

cpdef uint64_t gf_mul(uint64_t x, uint64_t y):
    '''Multiply two galois field integers'''
    if x == 0 or y == 0:
        return <uint64_t>0
    return gf_exp.data.as_ulonglongs[(gf_log.data.as_ulonglongs[x] + gf_log.data.as_ulonglongs[y]) % field_charac] # pyright: ignore[reportGeneralTypeIssues]
...
cpdef uint64_t gf_div(uint64_t x, uint64_t y):
    ...
    if x == 0:
        return <uint64_t>0
    return gf_exp.data.as_ulonglongs[(gf_log.data.as_ulonglongs[x] + field_charac - gf_log.data.as_ulonglongs[y]) % field_charac] # pyright: ignore[reportGeneralTypeIssues]

cpdef uint64_t gf_pow(uint64_t x, int64_t power):
    '''Power of x galois field integer'''
    return gf_exp.data.as_ulonglongs[(<int64_t>gf_log.data.as_ulonglongs[x] * power) % <int64_t>field_charac] # pyright: ignore[reportGeneralTypeIssues]

# ### Define bitwise carry-less operations as inner functions ###
# cpdef uint64_t cl_mult(uint64_t x, uint64_t y):
#     '''Bitwise carry-less multiplication on integers'''
#     cdef uint64_t z = <uint64_t>0
#     cdef uint64_t i = <uint64_t>0
#     while (y >> i) > 0:
#         if y & (1 << i):
#             z ^= x << i
#         i += 1
...
#     cdef uint64_t i
#     for i in range(dl1 - dl2, -1, -1): # pyright: ignore[reportGeneralTypeIssues]
...
# cpdef uint64_t gf_mult_noLUT_slow(uint64_t x, uint64_t y, uint64_t prim = <uint64_t>0):
...
cpdef uint64_t gf_mult_noLUT(uint64_t x, uint64_t y, uint64_t prim = <uint64_t>0, uint64_t field_charac_full = <uint64_t>256, bool carryless = True):
...
cpdef ca.array gf_poly_scale(const uint64_t[::1] p, uint64_t x):
    ...
    cdef ca.array out = ca.clone(uint64_array_template, <Py_ssize_t>p.shape[0], <bint>False)
    cdef uint64_t i
    for i in range(<uint64_t>p.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        out.data.as_ulonglongs[i] = gf_mul(<uint64_t>p[i], x) # pyright: ignore[reportGeneralTypeIssues]
    return out

cpdef ca.array gf_poly_add(const uint64_t[::1] p, const uint64_t[::1] q):
    '''Add two galois field polynomials'''
    assert p is not None and q is not None
    cdef uint64_t p_len = <uint64_t>p.shape[0]
    cdef uint64_t q_len = <uint64_t>q.shape[0]
    cdef ca.array r
    cdef uint64_t[::1] r_view
    cdef uint64_t i
    if q_len > p_len:
        r = ca.clone(uint64_array_template, <Py_ssize_t>q_len, <bint>False)
        r_view = r # pyright: ignore[reportGeneralTypeIssues]
        r_view[:] = q # pyright: ignore[reportGeneralTypeIssues]
        for i in range(p_len): # pyright: ignore[reportGeneralTypeIssues]
            r.data.as_ulonglongs[r_view.shape[0] - p_len + i] ^= <uint64_t>p[i] # pyright: ignore[reportGeneralTypeIssues]
    else:
        r = ca.clone(uint64_array_template, <Py_ssize_t>p_len, <bint>False)
        r_view = r # pyright: ignore[reportGeneralTypeIssues]
        r_view[:] = p # pyright: ignore[reportGeneralTypeIssues]
        for i in range(q_len): # pyright: ignore[reportGeneralTypeIssues]
            r.data.as_ulonglongs[r_view.shape[0] - q_len + i] ^= <uint64_t>q[i] # pyright: ignore[reportGeneralTypeIssues]
    return r
...
cpdef ca.array gf_poly_mul(const uint64_t[::1] p, const uint64_t[::1] q):
    ...
    cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>p.shape[0] + <Py_ssize_t>q.shape[0] - 1, <bint>True)
    # Precompute the logarithm of p
    cdef ca.array lp = ca.clone(uint64_array_template, <Py_ssize_t>p.shape[0], <bint>False)
    cdef uint64_t i
    for i in range(<uint64_t>p.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        lp.data.as_ulonglongs[i] = gf_log.data.as_ulonglongs[p[i]] # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef uint64_t j
    for j in range(<uint64_t>q.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        qj = q[j] # pyright: ignore[reportGeneralTypeIssues] optimization: load the coefficient once
        if qj != 0: # log(0) is undefined, we need to check that
            lq = gf_log.data.as_ulonglongs[qj] # pyright: ignore[reportGeneralTypeIssues] Optimization: precache the logarithm of the current coefficient of q
            for i in range(<uint64_t>p.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
                if p[i] != 0: # pyright: ignore[reportGeneralTypeIssues] log(0) is undefined, need to check that...
                    r.data.as_ulonglongs[i + j] ^= gf_exp[lp.data.as_ulonglongs[i] + lq] # pyright: ignore[reportGeneralTypeIssues] equivalent to: r[i + j] = gf_add(r[i + j], gf_mul(p[i], q[j]))
    return r

# cpdef ca.array gf_poly_mul_simple(const uint64_t[::1] p, const uint64_t[::1] q): # simple equivalent way of multiplying two polynomials without precomputation, but thus it's slower
#     '''Multiply two polynomials, inside Galois Field'''
#     assert p is not None and q is not None
#     # Pre-allocate the result array
#     cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>p.shape[0] + <Py_ssize_t>q.shape[0] - 1, <bint>False)
#     # Compute the polynomial multiplication (just like the outer product of two vectors, we multiply each coefficients of p with all coefficients of q)
#     cdef uint64_t i, j
#     for i in range(<uint64_t>p.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
#         for j in range(<uint64_t>q.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
#             r.data.as_ulonglongs[i + j] ^= gf_mul(<uint64_t>p[i], <uint64_t>q[j]) # pyright: ignore[reportGeneralTypeIssues] equivalent to: r[i + j] = gf_add(r[i + j], gf_mul(p[i], q[j])) -- you can see it's your usual polynomial multiplication
#     return r
...
cpdef ca.array gf_poly_neg(const uint64_t[::1] poly):
    ...
    cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>poly.shape[0], <bint>False)
    cdef uint64_t[::1] r_view = r # pyright: ignore[reportGeneralTypeIssues]
    r_view[...] = poly # pyright: ignore[reportGeneralTypeIssues]
    return r
...
cpdef tuple gf_poly_div(const uint64_t[::1] dividend, const uint64_t[::1] divisor):
    ...
    cdef ca.array msg_out = ca.clone(uint64_array_template, <Py_ssize_t>dividend.shape[0], <bint>False) # Copy the dividend list and pad with 0 where the ecc bytes will be computed
    cdef uint64_t[::1] msg_out_view = msg_out # pyright: ignore[reportGeneralTypeIssues]
    msg_out_view[...] = dividend # pyright: ignore[reportGeneralTypeIssues]
    # Cache lengths for faster access inside loops
    cdef uint64_t divisor_len = <uint64_t>divisor.shape[0]
    #normalizer = divisor[0] # precomputing for performance
    cdef uint64_t i, j, coef
    for i in range(dividend.shape[0] - (divisor_len - 1)): # pyright: ignore[reportGeneralTypeIssues]s
        #msg_out.data.as_ulonglongs[i] /= normalizer # pyright: ignore[reportGeneralTypeIssues] for general polynomial division (when polynomials are non-monic), the usual way of using synthetic division is to divide the divisor g(x) with its leading coefficient (call it a). In this implementation, this means:we need to compute: coef = msg_out[i] / gen[0]. For more infos, see http://en.wikipedia.org/wiki/Synthetic_division
        coef = msg_out.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues] precaching
        if coef != 0: # log(0) is undefined, so we need to avoid that case explicitly (and it's also a good optimization). In fact if you remove it, it should still work because gf_mul() will take care of the condition. But it's still a good practice to put the condition here.
            for j in range(1, divisor_len): # pyright: ignore[reportGeneralTypeIssues] in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient
                if divisor[j] != 0: # pyright: ignore[reportGeneralTypeIssues] log(0) is undefined
                    msg_out.data.as_ulonglongs[i + j] ^= gf_mul(<uint64_t>divisor[j], coef) # pyright: ignore[reportGeneralTypeIssues] equivalent to the more mathematically correct (but xoring directly is faster): msg_out[i + j] += -divisor[j] * coef
    ...
    cdef ca.array quotient = ca.clone(uint64_array_template, <Py_ssize_t>separator, <bint>False)
    cdef uint64_t[::1] quotient_view = quotient # pyright: ignore[reportGeneralTypeIssues]
    quotient_view[...] = msg_out_view[:separator] # pyright: ignore[reportGeneralTypeIssues]
    cdef ca.array remainder = ca.clone(uint64_array_template, <Py_ssize_t>msg_out_view.shape[0] - <Py_ssize_t>separator, <bint>False)
    cdef uint64_t[::1] remainder_view = remainder # pyright: ignore[reportGeneralTypeIssues]
    remainder_view[...] = msg_out_view[separator:] # pyright: ignore[reportGeneralTypeIssues]
    return quotient, remainder

cpdef ca.array gf_poly_square(const uint64_t[::1] poly):  # pragma: no cover
    ...
    assert poly is not None
    cdef uint64_t length = <uint64_t>poly.shape[0]
    cdef ca.array out = ca.clone(uint64_array_template, <Py_ssize_t>length * 2 - 1, <bint>True)
    cdef uint64_t i, p, k
    for i in range(length - 1): # pyright: ignore[reportGeneralTypeIssues]
        p = <uint64_t>poly[i] # pyright: ignore[reportGeneralTypeIssues]
        k = i * 2
        if p != 0:
            #out.data.as_ulonglongs[k] = gf_exp.data.as_ulonglongs[(2 * gf_log.data.as_ulonglongs[p]) % field_charac] # pyright: ignore[reportGeneralTypeIssues] not necessary to modulo (2^r)-1 since gf_exp is duplicated up to 510.
            out.data.as_ulonglongs[k] = gf_exp.data.as_ulonglongs[gf_log.data.as_ulonglongs[p] * 2] # pyright: ignore[reportGeneralTypeIssues]
        #else: # not necessary since the output is already initialized to an array of 0
            #out.data.as_ulonglongs[k] = 0 # pyright: ignore[reportGeneralTypeIssues]
    out.data.as_ulonglongs[length * 2 - 2] = gf_exp.data.as_ulonglongs[gf_log.data.as_ulonglongs[<uint64_t>poly[length - 1]] * 2] # pyright: ignore[reportGeneralTypeIssues]
    if out.data.as_ulonglongs[0] == 0: # pyright: ignore[reportGeneralTypeIssues]
        out.data.as_ulonglongs[0] = <uint64_t>poly[1] * 2 - 1 # pyright: ignore[reportGeneralTypeIssues]
    return out

cpdef uint64_t gf_poly_eval(const uint64_t[::1] poly, uint64_t x):
    '''Evaluates a polynomial in GF(2^p) given the value for x. This is based on Horner's scheme for maximum efficiency.'''
    assert poly is not None
    cdef uint64_t y = <uint64_t>poly[0] # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t i
    for i in range(1, <uint64_t>poly.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        y = gf_mul(y, x) ^ <uint64_t>poly[i] # pyright: ignore[reportGeneralTypeIssues]
    return y


################### REED-SOLOMON ENCODING ###################

cpdef ca.array rs_generator_poly(uint64_t nsym, uint64_t fcr, uint64_t generator = <uint64_t>2):
    '''Generate an irreducible generator polynomial (necessary to encode a message into Reed-Solomon)'''
    cdef ca.array g = ca.clone(uint64_array_template, <Py_ssize_t>1, <bint>False)
    g.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t i
    cdef ca.array t = ca.clone(uint64_array_template, <Py_ssize_t>2, <bint>False)
    t.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
    for i in range(nsym): # pyright: ignore[reportGeneralTypeIssues]
        t.data.as_ulonglongs[1] = gf_pow(generator, i + fcr) # pyright: ignore[reportGeneralTypeIssues]
        g = gf_poly_mul(g, t) # pyright: ignore[reportGeneralTypeIssues]
    return g

cpdef list rs_generator_poly_all(uint64_t max_nsym, uint64_t fcr, uint64_t generator = <uint64_t>2):
    ...
    cdef uint64_t nsym
    for nsym in range(max_nsym): # pyright: ignore[reportGeneralTypeIssues]
        g_all.append(rs_generator_poly(nsym, fcr, generator))
    return g_all

# cpdef ca.array rs_simple_encode_msg(const uint64_t[::1] msg_in, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2, const uint64_t[::1] gen = None): # pyright: ignore[reportGeneralTypeIssues]
...
#     if (msg_in.shape[0] + nsym) > field_charac:
#         raise ValueError("Message is too long (%i when max is %i)" % (msg_in.shape[0] + nsym, field_charac))
#     if gen is None:
#         gen = rs_generator_poly(nsym, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]

#     # Pad the message, then divide it by the irreducible generator polynomial
#     cdef ca.array dividend = ca.clone(uint64_array_template, <Py_ssize_t>msg_in.shape[0] + <Py_ssize_t>gen.shape[0] - 1, <bint>True)
#     cdef uint64_t[::1] dividend_view = dividend # pyright: ignore[reportGeneralTypeIssues]
#     dividend_view[:msg_in.shape[0]] = msg_in # pyright: ignore[reportGeneralTypeIssues]
#     _, remainder = gf_poly_div(dividend_view, gen) # pyright: ignore[reportGeneralTypeIssues]
...
#     cdef ca.array msg_out = ca.clone(uint64_array_template, <Py_ssize_t>msg_in.shape[0] + <Py_ssize_t>len(remainder), <bint>False)
#     cdef uint64_t[::1] msg_out_view = msg_out # pyright: ignore[reportGeneralTypeIssues]
#     msg_out_view[:msg_in.shape[0]] = msg_in # pyright: ignore[reportGeneralTypeIssues]
#     msg_out_view[msg_in.shape[0]:] = remainder # pyright: ignore[reportGeneralTypeIssues]
#     # Return the codeword
#     return msg_out

cpdef ca.array rs_encode_msg(const uint64_t[::1] msg_in, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2, const uint64_t[::1] gen = None): # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef ca.array msg_out = ca.clone(uint64_array_template, <Py_ssize_t>msg_in.shape[0] + <Py_ssize_t>gen.shape[0] - 1, <bint>True) # init msg_out with the values inside msg_in and pad with len(gen)-1 bytes (which is the number of ecc symbols).
    cdef uint64_t[::1] msg_out_view = msg_out # pyright: ignore[reportGeneralTypeIssues]
    msg_out_view[:msg_in.shape[0]] = msg_in # pyright: ignore[reportGeneralTypeIssues]

    # Precompute the logarithm of every items in the generator
    cdef ca.array lgen = ca.clone(uint64_array_template, <Py_ssize_t>gen.shape[0], <bint>False)
    cdef uint64_t i
    for i in range(<uint64_t>gen.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        lgen.data.as_ulonglongs[i] = gf_log.data.as_ulonglongs[<uint64_t>gen[i]] # pyright: ignore[reportGeneralTypeIssues]

    # Cache lengths for faster access inside loops
    cdef uint64_t msg_in_len = <uint64_t>msg_in.shape[0]
    cdef uint64_t gen_len = <uint64_t>gen.shape[0]
    ...
    for i in range(msg_in_len): # pyright: ignore[reportGeneralTypeIssues]
        coef = msg_out.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues] Note that it's msg_out here, not msg_in. Thus, we reuse the updated value at each iteration (this is how Synthetic Division works: instead of storing in a temporary register the intermediate values, we directly commit them to the output).
        ...
        if coef != 0: # log(0) is undefined, so we need to manually check for this case. There's no need to check the divisor here because we know it can't be 0 since we generated it.
            lcoef = gf_log.data.as_ulonglongs[coef] # pyright: ignore[reportGeneralTypeIssues] precaching

            for j in range(1, gen_len): # pyright: ignore[reportGeneralTypeIssues] in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient (which is here useless since the divisor, the generator polynomial, is always monic)
                ...
                msg_out.data.as_ulonglongs[i + j] ^= gf_exp.data.as_ulonglongs[lcoef + lgen.data.as_ulonglongs[j]] # pyright: ignore[reportGeneralTypeIssues] optimization, equivalent to gf_mul(gen[j], msg_out[i]) and we just substract it to msg_out[i+j] (but since we are in GF256, it's equivalent to an addition and to an XOR). In other words, this is simply a "multiply-accumulate operation"

    # Recopy the original message bytes (overwrites the part where the quotient was computed)
    msg_out_view[:msg_in_len] = msg_in # pyright: ignore[reportGeneralTypeIssues] equivalent to c = mprime - b, where mprime is msg_in padded with [0]*nsym
    return msg_out
...
cpdef ca.array inverted(const uint64_t[::1] msg):
    ...
    cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>msg.shape[0], <bint>False)
    cdef uint64_t[::1] r_view = r # pyright: ignore[reportGeneralTypeIssues]
    r_view[:] = msg[::-1] # pyright: ignore[reportGeneralTypeIssues]
    return r

cpdef ca.array rs_calc_syndromes(const uint64_t[::1] msg, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2):
    ...
    cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>nsym + 1, <bint>False)
    r.data.as_ulonglongs[0] = <uint64_t>0 # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t i
    for i in range(nsym): # pyright: ignore[reportGeneralTypeIssues]
        r.data.as_ulonglongs[i + 1] = gf_poly_eval(msg, gf_pow(generator, i + fcr)) # pyright: ignore[reportGeneralTypeIssues]
    return r

cpdef ca.array rs_correct_errata(const uint64_t[::1] msg_in, const uint64_t[::1] synd, const uint64_t[::1] err_pos, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2): # err_pos is a list of the positions of the errors/erasures/errata
    '''Forney algorithm, computes the values (error magnitude) to correct the input message.'''
    assert msg_in is not None and synd is not None and err_pos is not None
    cdef ca.array msg = ca.clone(uint64_array_template, <Py_ssize_t>msg_in.shape[0], <bint>False)
    cdef uint64_t[::1] msg_view = msg # pyright: ignore[reportGeneralTypeIssues]
    msg_view[...] = msg_in # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef ca.array coef_pos = ca.clone(uint64_array_template, <Py_ssize_t>err_pos.shape[0], <bint>False) # need to convert the positions to coefficients degrees for the errata locator algo to work (eg: instead of [0, 1, 2] it will become [len(msg)-1, len(msg)-2, len(msg) -3]), also can use a _bytearray() because length of message - and hence length of coef_pos - will always be constrained by the Galois Field.
    cdef uint64_t i
    for i in range(<uint64_t>err_pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        coef_pos.data.as_ulonglongs[i] = msg_view.shape[0] - 1 - <uint64_t>err_pos[i] # pyright: ignore[reportGeneralTypeIssues]
    cdef ca.array err_loc = rs_find_errata_locator(coef_pos, generator) # pyright: ignore[reportGeneralTypeIssues]
    # calculate errata evaluator polynomial (often called Omega or Gamma in academic papers)
    cdef ca.array err_eval = inverted(rs_find_error_evaluator(inverted(synd), err_loc, <uint64_t>len(err_loc) - 1)) # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef ca.array X = ca.clone(uint64_array_template, <Py_ssize_t>len(coef_pos), <bint>False) # pyright: ignore[reportGeneralTypeIssues] will store the position of the errors
    cdef uint64_t l
    for i in range(<uint64_t>len(coef_pos)): # pyright: ignore[reportGeneralTypeIssues]
        l = field_charac - coef_pos.data.as_ulonglongs[i] # pyright: ignore[reportGeneralTypeIssues]
        X.data.as_ulonglongs[i] = gf_pow(generator, -<int64_t>l) # pyright: ignore[reportGeneralTypeIssues]

    # Forney algorithm: compute the magnitudes
    cdef ca.array E = ca.clone(uint64_array_template, <Py_ssize_t>len(msg), <bint>True) # pyright: ignore[reportGeneralTypeIssues] will store the values that need to be corrected (substracted) to the message containing errors. This is sometimes called the error magnitude polynomial.
    cdef uint64_t X_len = <uint64_t>len(X) # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t Xi_inv, err_loc_prime, j, y, magnitude
    for i in range(X_len): # pyright: ignore[reportGeneralTypeIssues]

        Xi_inv = gf_inverse(X.data.as_ulonglongs[i]) # pyright: ignore[reportGeneralTypeIssues]
        ...
        err_loc_prime = <uint64_t>1
        for j in range(X_len): # pyright: ignore[reportGeneralTypeIssues]
            if j != i:
                err_loc_prime = gf_mul(err_loc_prime, gf_sub(<uint64_t>1, gf_mul(Xi_inv, X.data.as_ulonglongs[j]))) # pyright: ignore[reportGeneralTypeIssues]
        ...
        y = gf_poly_eval(inverted(err_eval), Xi_inv) # pyright: ignore[reportGeneralTypeIssues] numerator of the Forney algorithm (errata evaluator evaluated)
        y = gf_mul(gf_pow(X.data.as_longlongs[i], 1 - <int64_t>fcr), y) # pyright: ignore[reportGeneralTypeIssues] adjust to fcr parameter
        ...
        E.data.as_ulonglongs[<uint64_t>err_pos[i]] = magnitude # pyright: ignore[reportGeneralTypeIssues] store the magnitude for this error into the magnitude polynomial
    ...
    msg = gf_poly_add(msg, E) # pyright: ignore[reportGeneralTypeIssues] equivalent to Ci = Ri - Ei where Ci is the correct message, Ri the received (senseword) message, and Ei the errata magnitudes (minus is replaced by XOR since it's equivalent in GF(2^p)). So in fact here we substract from the received message the errors magnitude, which logically corrects the value to what it should be.
    return msg

cpdef ca.array rs_find_error_locator(const uint64_t[::1] synd, uint64_t nsym, const uint64_t[::1] erase_loc = None, uint64_t erase_count = 0): # pyright: ignore[reportGeneralTypeIssues]
    ...
    if erase_loc is not None: # if the erasure locator polynomial is supplied, we init with its value, so that we include erasures in the final locator polynomial
        err_loc = ca.clone(uint64_array_template, <Py_ssize_t>erase_loc.shape[0], <bint>False)
        err_loc_view = err_loc # pyright: ignore[reportGeneralTypeIssues]
        err_loc_view[:] = err_loc # pyright: ignore[reportGeneralTypeIssues]
        old_loc = ca.clone(uint64_array_template, <Py_ssize_t>erase_loc.shape[0], <bint>False)
        old_loc_view = old_loc # pyright: ignore[reportGeneralTypeIssues]
        old_loc_view[:] = erase_loc # pyright: ignore[reportGeneralTypeIssues]
    else:
        err_loc = ca.clone(uint64_array_template, <Py_ssize_t>1, <bint>False) # This is the main variable we want to fill, also called Sigma in other notations or more formally the errors/errata locator polynomial.
        err_loc.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
        old_loc = ca.clone(uint64_array_template, <Py_ssize_t>1, <bint>False) # BM is an iterative algorithm, and we need the errata locator polynomial of the previous iteration in order to update other necessary variables.
        old_loc.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef uint64_t synd_shift = <uint64_t>0
    if <uint64_t>synd.shape[0] > nsym:
        synd_shift = synd.shape[0] - nsym

    cdef uint64_t K, delta, j, err_loc_len
    cdef ca.array new_loc
    for i in range(nsym - erase_count): # pyright: ignore[reportGeneralTypeIssues] generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
        ...
        delta = <uint64_t>synd[K] # pyright: ignore[reportGeneralTypeIssues]
        err_loc_len = <uint64_t>len(err_loc) # pyright: ignore[reportGeneralTypeIssues]
        for j in range(1, <uint64_t>len(err_loc)):  # pyright: ignore[reportGeneralTypeIssues] range 1:256 is important: if you use range 0:255, if the last byte of the ecc symbols is corrupted, it won't be correctable! You need to use the range 1,256 to include this last byte.
            delta ^= gf_mul(err_loc.data.as_ulonglongs[err_loc_len - (j + 1)], <uint64_t>synd[K - j]) # pyright: ignore[reportGeneralTypeIssues] delta is also called discrepancy. Here we do a partial polynomial multiplication (ie, we compute the polynomial multiplication only for the term of degree K). Should be equivalent to brownanrs.polynomial.mul_at().
        #print "delta", K, delta, list(gf_poly_mul(inverted(err_loc), synd)) # debugline

        # Shift polynomials to compute the next degree
        ca.resize(old_loc, <Py_ssize_t>len(old_loc) + 1) # pyright: ignore[reportGeneralTypeIssues]
        old_loc.data.as_longlongs[len(old_loc) - 1] = 0 # pyright: ignore[reportGeneralTypeIssues]

        # Iteratively estimate the errata locator and evaluator polynomials
        if delta != 0: # Update only if there's a discrepancy
            if len(old_loc) > len(err_loc): # pyright: ignore[reportGeneralTypeIssues] Rule B (rule A is implicitly defined because rule A just says that we skip any modification for this iteration)
                ...
                new_loc = gf_poly_scale(old_loc, delta) # pyright: ignore[reportGeneralTypeIssues]
                old_loc = gf_poly_scale(err_loc, gf_inverse(delta)) # pyright: ignore[reportGeneralTypeIssues] effectively we are doing err_loc * 1/delta = err_loc // delta
                err_loc = new_loc
            ...
            err_loc = gf_poly_add(err_loc, gf_poly_scale(old_loc, delta)) # pyright: ignore[reportGeneralTypeIssues]

    # Check if the result is correct, that there's not too many errors to correct
    cdef uint64_t[::1] new_loc_view
    for i in range(<uint64_t>len(err_loc)): # pyright: ignore[reportGeneralTypeIssues] drop leading 0s, else errs will not be of the correct size. This does not use functional closures (ie, lambdas), which Cython and JIT compilers cannot optimize, such as `err_loc = list(itertools.dropwhile(lambda x: x == 0, err_loc))`
        if err_loc.data.as_ulonglongs[i] != 0: # pyright: ignore[reportGeneralTypeIssues]
            new_loc = ca.clone(uint64_array_template, <Py_ssize_t>len(err_loc) - i, <bint>False) # pyright: ignore[reportGeneralTypeIssues]
            new_loc_view = new_loc # pyright: ignore[reportGeneralTypeIssues]
            err_loc_view = err_loc # pyright: ignore[reportGeneralTypeIssues]
            new_loc_view[:] = err_loc_view[i:] # pyright: ignore[reportGeneralTypeIssues]
            err_loc = new_loc
            break

    cdef uint64_t errs = <uint64_t>len(err_loc) - 1  # pyright: ignore[reportGeneralTypeIssues] -1 because range is 1:256, it's offset by 1, it's not 0:255, hence the length would be overestimated without -1
...
cpdef ca.array rs_find_errata_locator(const uint64_t[::1] e_pos, uint64_t generator = <uint64_t>2):
    ...
    cdef ca.array e_loc = ca.clone(uint64_array_template, <Py_ssize_t>1, <bint>False) # just to init because we will multiply, so it must be 1 so that the multiplication starts correctly without nulling any term
    e_loc.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef uint64_t i
    cdef ca.array one = ca.clone(uint64_array_template, <Py_ssize_t>1, <bint>False)
    one.data.as_ulonglongs[0] = <uint64_t>1 # pyright: ignore[reportGeneralTypeIssues]
    cdef ca.array t = ca.clone(uint64_array_template, <Py_ssize_t>2, <bint>False)
    t.data.as_ulonglongs[1] = <uint64_t>0 # pyright: ignore[reportGeneralTypeIssues]
    for i in range(<uint64_t>e_pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        t.data.as_ulonglongs[0] = gf_pow(generator, <uint64_t>e_pos[i]) # pyright: ignore[reportGeneralTypeIssues]
        e_loc = gf_poly_mul(e_loc, gf_poly_add(one, t)) # pyright: ignore[reportGeneralTypeIssues]
    return e_loc
...
cpdef ca.array rs_find_error_evaluator(const uint64_t[::1] synd, const uint64_t[::1] err_loc, uint64_t nsym):
    ...
    cdef ca.array remainder = gf_poly_mul(synd, err_loc) # first multiply the syndromes with the errata locator polynomial
    cdef ca.array r = ca.clone(uint64_array_template, <Py_ssize_t>nsym + 1, <bint>False)
    cdef uint64_t[::1] r_view = r # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t[::1] remainder_view = remainder # pyright: ignore[reportGeneralTypeIssues]
    r_view[:] = remainder_view[<uint64_t>remainder_view.shape[0] - (nsym + 1):] # pyright: ignore[reportGeneralTypeIssues]
    return r

cpdef ca.array rs_find_errors(const uint64_t[::1] err_loc, uint64_t nmess, uint64_t generator = <uint64_t>2):
    ...
    cdef ca.array err_pos = ca.clone(uint64_array_template, <Py_ssize_t>nmess, <bint>False)
    cdef uint64_t err_pos_size = <uint64_t>0
    cdef uint64_t i
    for i in range(nmess): # pyright: ignore[reportGeneralTypeIssues] normally we should try all 2^8 possible values, but here we optimize to just check the interesting symbols
        if gf_poly_eval(err_loc, gf_pow(generator, nmess - 1 - i)) == 0: # It's a 0? Bingo, it's a root of the error locator polynomial, in other terms this is the location of an error
            err_pos.data.as_ulonglongs[err_pos_size] = i # pyright: ignore[reportGeneralTypeIssues]
            err_pos_size += 1
    ca.resize(err_pos, <Py_ssize_t>err_pos_size)
    ...
    if <uint64_t>len(err_pos) != errs: # pyright: ignore[reportGeneralTypeIssues]
...
cpdef ca.array rs_forney_syndromes(const uint64_t[::1] synd, const uint64_t[::1] pos, uint64_t nmess, uint64_t generator = <uint64_t>2):
    ...
    cdef ca.array erase_pos_reversed = ca.clone(uint64_array_template, <Py_ssize_t>pos.shape[0], <bint>False) # prepare the coefficient degree positions (instead of the erasures positions)
    cdef uint64_t i
    for i in range(<uint64_t>pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        erase_pos_reversed.data.as_ulonglongs[i] = nmess - 1 - <uint64_t>pos[i] # pyright: ignore[reportGeneralTypeIssues]
    # Optimized method, all operations are inlined
    cdef ca.array fsynd = ca.clone(uint64_array_template, <Py_ssize_t>synd.shape[0] - 1, <bint>False) # make a copy and trim the first coefficient which is always 0 by definition
    cdef uint64_t[::1] fsynd_view = fsynd # pyright: ignore[reportGeneralTypeIssues]
    fsynd_view[...] = synd[1:] # pyright: ignore[reportGeneralTypeIssues]
    cdef uint64_t x, j
    for i in range(<uint64_t>pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
        pass
        x = gf_pow(generator, erase_pos_reversed.data.as_ulonglongs[i]) # pyright: ignore[reportGeneralTypeIssues]
        for j in range(<uint64_t>len(fsynd) - 1): # pyright: ignore[reportGeneralTypeIssues]
            fsynd.data.as_ulonglongs[j] = gf_mul(fsynd.data.as_ulonglongs[j], x) ^ fsynd.data.as_ulonglongs[j + 1] # pyright: ignore[reportGeneralTypeIssues]
    ...
    #erase_loc = rs_find_errata_locator(erase_pos_reversed, generator) # computing the erasures locator polynomial
...
cpdef tuple rs_correct_msg(const uint64_t[::1] msg_in, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2, const uint64_t[::1] erase_pos = None, bool only_erasures = False): # pyright: ignore[reportGeneralTypeIssues]
    ...
    if <uint64_t>msg_in.shape[0] > field_charac:
        ...
        raise ValueError("Message is too long (%i when max is %i)" % (msg_in.shape[0], field_charac))

    cdef ca.array msg_out = ca.clone(uint64_array_template, <Py_ssize_t>msg_in.shape[0], <bint>False) # copy of message
    cdef uint64_t[::1] msg_out_view = msg_out # pyright: ignore[reportGeneralTypeIssues]
    msg_out_view[:] = msg_in # pyright: ignore[reportGeneralTypeIssues]
    ...
    if erase_pos is None:
        erase_pos = ca.clone(uint64_array_template, <Py_ssize_t>0, <bint>False) # pyright: ignore[reportGeneralTypeIssues]
    else:
        for i in range(<uint64_t>erase_pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
            msg_out.data.as_ulonglongs[<uint64_t>erase_pos[i]] = <uint64_t>0 # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef ca.array synd = rs_calc_syndromes(msg_out, nsym, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]
    ...
    cdef bool flag = True
    for i in range(<uint64_t>len(synd)): # pyright: ignore[reportGeneralTypeIssues]
        if synd.data.as_ulonglongs[i]: # pyright: ignore[reportGeneralTypeIssues]
            flag = False
            break
    cdef ca.array content, ecc, wrong_pos
    cdef uint64_t[::1] content_view, ecc_view, wrong_pos_view
    if flag:
        content = ca.clone(uint64_array_template, <Py_ssize_t>msg_out_view.shape[0] - <Py_ssize_t>nsym, <bint>False)
        content_view = content # pyright: ignore[reportGeneralTypeIssues]
        content_view[:] = msg_out_view[:-nsym] # pyright: ignore[reportGeneralTypeIssues]
        ecc = ca.clone(uint64_array_template, <Py_ssize_t>nsym, <bint>False)
        ecc_view = ecc # pyright: ignore[reportGeneralTypeIssues]
        ecc_view[:] = msg_out_view[-nsym:] # pyright: ignore[reportGeneralTypeIssues]
        wrong_pos = ca.clone(uint64_array_template, <Py_ssize_t>erase_pos.shape[0], <bint>False)
        wrong_pos_view = wrong_pos # pyright: ignore[reportGeneralTypeIssues]
        wrong_pos_view[:] = erase_pos # pyright: ignore[reportGeneralTypeIssues]
        return content, ecc, wrong_pos # no errors

    # Find errors locations
    cdef ca.array err_pos, fsynd, err_loc
    if only_erasures:
        err_pos = ca.clone(uint64_array_template, <Py_ssize_t>0, <bint>False)
    else:
        ...
        fsynd = rs_forney_syndromes(synd, erase_pos, <uint64_t>len(msg_out), generator) # pyright: ignore[reportGeneralTypeIssues]
        # compute the error locator polynomial using Berlekamp-Massey
        err_loc = rs_find_error_locator(fsynd, nsym, erase_count = <uint64_t>erase_pos.shape[0]) # pyright: ignore[reportGeneralTypeIssues]
        # locate the message errors using Chien search (or bruteforce search)
        err_pos = rs_find_errors(inverted(err_loc), <uint64_t>len(msg_out), generator) # pyright: ignore[reportGeneralTypeIssues]
        if err_pos is None:
    ...
    cdef uint64_t[::1] err_pos_view = err_pos # pyright: ignore[reportGeneralTypeIssues]
    wrong_pos = ca.clone(uint64_array_template, <Py_ssize_t>erase_pos.shape[0] + <Py_ssize_t>len(err_pos), <bint>False) # pyright: ignore[reportGeneralTypeIssues]
    wrong_pos_view = wrong_pos # pyright: ignore[reportGeneralTypeIssues]
    wrong_pos_view[:erase_pos.shape[0]] = erase_pos # pyright: ignore[reportGeneralTypeIssues]
    wrong_pos_view[erase_pos.shape[0]:] = err_pos_view # pyright: ignore[reportGeneralTypeIssues]
    msg_out = rs_correct_errata(msg_out, synd, wrong_pos, fcr, generator) # pyright: ignore[reportGeneralTypeIssues] note that we here use the original syndrome, not the forney syndrome (because we will correct both errors and erasures, so we need the full syndrome)
    # check if the final message is fully repaired
    synd = rs_calc_syndromes(msg_out, nsym, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]
    flag = True
    for i in range(<uint64_t>len(synd)): # pyright: ignore[reportGeneralTypeIssues]
        if synd.data.as_ulonglongs[i] != 0: # pyright: ignore[reportGeneralTypeIssues]
            flag = False
            break
    if not flag:
        raise ReedSolomonError("Could not correct message")
    # return the successfully decoded message
    msg_out_view = msg_out # pyright: ignore[reportGeneralTypeIssues]
    content = ca.clone(uint64_array_template, <Py_ssize_t>msg_out_view.shape[0] - <Py_ssize_t>nsym, <bint>False)
    content_view = content # pyright: ignore[reportGeneralTypeIssues]
    content_view[:] = msg_out_view[:-nsym] # pyright: ignore[reportGeneralTypeIssues]
    ecc = ca.clone(uint64_array_template, <Py_ssize_t>nsym, <bint>False)
...
# cpdef tuple rs_correct_msg_nofsynd(const uint64_t[::1] msg_in, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2, const uint64_t[::1] erase_pos = None, bool only_erasures = False): # pyright: ignore[reportGeneralTypeIssues]
...
#     cdef uint64_t erase_count = <uint64_t>erase_pos.shape[0]
#     cdef uint64_t msg_out_len = <uint64_t>len(msg_out) # pyright: ignore[reportGeneralTypeIssues] cache to avoid recalculations inside loop
#     cdef ca.array erase_pos_reversed = ca.clone(uint64_array_template, <Py_ssize_t>erase_pos.shape[0], <bint>False)
#     for i in range(<uint64_t>erase_pos.shape[0]): # pyright: ignore[reportGeneralTypeIssues]
#         erase_pos_reversed.data.as_ulonglongs[i] = msg_out_len - 1 - <uint64_t>erase_pos[i] # pyright: ignore[reportGeneralTypeIssues]
#     cdef ca.array erase_loc = rs_find_errata_locator(erase_pos_reversed, generator) # pyright: ignore[reportGeneralTypeIssues]
#     #cdef ca.array erase_eval = rs_find_error_evaluator(inverted(synd), erase_loc, <uint64_t>len(erase_loc) - 1) # pyright: ignore[reportGeneralTypeIssues]

#     # prepare errors/errata locator polynomial
#     cdef ca.array err_loc
#     if only_erasures:
#         err_loc = inverted(erase_loc) # pyright: ignore[reportGeneralTypeIssues]
#         #err_eval = inverted(erase_eval) # pyright: ignore[reportGeneralTypeIssues]
#     else:
#         err_loc = rs_find_error_locator(synd, nsym, erase_loc, erase_count) # pyright: ignore[reportGeneralTypeIssues]
#         err_loc = inverted(err_loc) # pyright: ignore[reportGeneralTypeIssues]
#         #err_eval = inverted(rs_find_error_evaluator(inverted(synd), inverted(err_loc), <uint64_t>len(err_loc) - 1)) # pyright: ignore[reportGeneralTypeIssues] find error/errata evaluator polynomial (not really necessary since we already compute it at the same time as the error locator poly in BM)

#     # locate the message errors
#     cdef ca.array err_pos = rs_find_errors(err_loc, <uint64_t>len(msg_out), generator) # pyright: ignore[reportGeneralTypeIssues] find the roots of the errata locator polynomial (ie: the positions of the errors/errata)
#     if err_pos is None:
#         raise ReedSolomonError("Could not locate error")

#     # compute errata evaluator and errata magnitude polynomials, then correct errors and erasures
#     msg_out = rs_correct_errata(msg_out, synd, err_pos, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]
#     # check if the final message is fully repaired
#     synd = rs_calc_syndromes(msg_out, nsym, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]
#     flag = True
#     for i in range(<uint64_t>len(synd)): # pyright: ignore[reportGeneralTypeIssues]
...
cpdef bool rs_check(const uint64_t[::1] msg, uint64_t nsym, uint64_t fcr = <uint64_t>0, uint64_t generator = <uint64_t>2):
    ...
    assert msg is not None
    cdef ca.array synd = rs_calc_syndromes(msg, nsym, fcr, generator)
    for i in range(<uint64_t>len(synd)): # pyright: ignore[reportGeneralTypeIssues]
        if synd.data.as_ulonglongs[i]: # pyright: ignore[reportGeneralTypeIssues]
            return False
    return True
...
cdef class RSCodec:
    ...
    def __init__(self, nsym: int = 10, nsize: int = 255, fcr: int = 0, prim: int = 0x11d, generator: int = 2, c_exp=8, single_gen: bool = True) -> None:
        ...
        if c_exp != 8 and prim == 0x11d:  # prim was not correctly defined, find one
            prim = <int>find_prime_polys(generator, c_exp, True, True).data.as_ulonglongs[0] # pyright: ignore[reportGeneralTypeIssues]
        ...
        # Memorize variables
        self.nsym = <uint64_t>nsym # number of ecc symbols (ie, the repairing rate will be r=(nsym/2)/nsize, so for example if you have nsym=5 and nsize=10, you have a rate r=0.25, so you can correct up to 0.25% errors (or exactly 2 symbols out of 10), and 0.5% erasures (5 symbols out of 10).
        self.nsize = <uint64_t>nsize # maximum length of one chunk (ie, message + ecc symbols after encoding, for the message alone it's nsize-nsym)
        self.fcr = <uint64_t>fcr # first consecutive root, can be any value between 0 and (2**c_exp)-1
        self.prim = <uint64_t>prim # prime irreducible polynomial, use find_prime_polys() to find a prime poly
        self.generator = <uint64_t>generator # generator integer, must be prime
        self.c_exp = <uint64_t>c_exp # exponent of the field's characteristic. This both defines the maximum value per symbol and the maximum length of one chunk. By default it's GF(2^8), do not change if you're not sure what it means.
        self.single_gen = single_gen

        # Initialize the look-up tables for easy and quick multiplication/division
        self.gf_log, self.gf_exp, self.field_charac = init_tables(<uint64_t>prim, <uint64_t>generator, <uint64_t>c_exp)
        # Precompute the generator polynomials
        if single_gen:
            self.gen = rs_generator_poly(<uint64_t>nsym, <uint64_t>fcr, <uint64_t>generator)
        else:
            self.gens = rs_generator_poly_all(<uint64_t>nsize, <uint64_t>fcr, <uint64_t>generator)
    ...
    def __save_globals(self) -> None:
        global gf_log, gf_exp, field_charac
        self.__globals_temp = (gf_log, gf_exp, field_charac)

    def __set_globals(self) -> None:
        global gf_log, gf_exp, field_charac
        gf_log, gf_exp, field_charac = self.gf_log, self.gf_exp, self.field_charac

    def __restore_globals(self) -> None:
        global gf_log, gf_exp, field_charac
        gf_log, gf_exp, field_charac = self.__globals_temp # pyright: ignore[reportGeneralTypeIssues]
    ...
    def encode(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[pa.array[int], 'Q']:
        ...
        if self.single_gen:
            if nsym_c != self.nsym:
                gen = rs_generator_poly(nsym_c, <uint64_t>fcr, <uint64_t>generator)
            else:
                gen = self.gen
        else:
        ...
        cdef uint64_t[::1] data_array_view = data_array # pyright: ignore[reportGeneralTypeIssues]

        # Calculate chunk size and total number of chunks for looping
        cdef uint64_t chunk_size = nsize - nsym_c
        cdef uint64_t total_chunks = (<uint64_t>len(data_array) - 1) // chunk_size + 1 # pyright: ignore[reportGeneralTypeIssues] need to convert to floats first to get an accurate floating division, or else we assume implicit conversion and it will cause an error on Python 2

        # Preallocate output array
        cdef ca.array enc = ca.clone(uint64_array_template, <Py_ssize_t>nsize * total_chunks, <bint>False) # pre-allocate array and we will overwrite data in it, much faster than extending
        cdef uint64_t[::1] enc_view = enc # pyright: ignore[reportGeneralTypeIssues]
        # Chunking loop
        cdef uint64_t i
        cdef ca.array r
        cdef uint64_t[::1] r_view
        for i in range(total_chunks): # pyright: ignore[reportGeneralTypeIssues]
            # Encode this chunk and update a slice of the output bytearray, much more efficient than extending an array constantly
            r = rs_encode_msg(data_array_view[chunk_size * i:chunk_size * (i + 1)], nsym_c, fcr, generator, gen) # pyright: ignore[reportGeneralTypeIssues]
            r_view = r # pyright: ignore[reportGeneralTypeIssues]
            enc_view[nsize * i:nsize * i + r_view.shape[0]] = r_view # pyright: ignore[reportGeneralTypeIssues]
            if i == total_chunks - 1:
                ca.resize(enc, <Py_ssize_t>nsize * i + r_view.shape[0])

        self.__restore_globals() # just incase someone is using the proceedural API at the same time.

        return enc # pyright: ignore[reportGeneralTypeIssues]
    ...
    def decode(self, data: Iterable[int], nsym: Optional[int] = None, erase_pos: Optional[Iterable[int]] = None, only_erasures: bool = False) -> tuple[Annotated[pa.array[int], 'Q'], Annotated[pa.array[int], 'Q'], Annotated[pa.array[int], 'Q']]:
        ...
        cdef ca.array dec = ca.clone(uint64_array_template, <Py_ssize_t>nmes * total_chunks, <bint>False) # pre-allocate array and we will overwrite data in it, much faster than extending
        cdef uint64_t[::1] dec_view = dec # pyright: ignore[reportGeneralTypeIssues]
        cdef ca.array dec_full = ca.clone(uint64_array_template, <Py_ssize_t>nsize * total_chunks, <bint>False)
        cdef uint64_t[::1] dec_full_view = dec_full # pyright: ignore[reportGeneralTypeIssues]
        cdef ca.array errata_pos_all = ca.clone(uint64_array_template, <Py_ssize_t>nsize * total_chunks, <bint>False)
        cdef uint64_t errata_pos_all_view_size = <uint64_t>0
        ...
        for i in range(total_chunks):  # pyright: ignore[reportGeneralTypeIssues] Split the long message in a chunk
            ...
            if has_erase_pos_array:
                ...
                e_pos = ca.clone(uint64_array_template, <Py_ssize_t>len(erase_pos_array), <bint>False) # pyright: ignore[reportGeneralTypeIssues]
                e_pos_size = <uint64_t>0
                for j in range(<uint64_t>len(erase_pos_array)): # pyright: ignore[reportGeneralTypeIssues]
                    if erase_pos_array.data.as_ulonglongs[j] < nsize: # pyright: ignore[reportGeneralTypeIssues]
                        e_pos.data.as_ulonglongs[e_pos_size] = erase_pos_array.data.as_ulonglongs[j] # pyright: ignore[reportGeneralTypeIssues]
                        e_pos_size += 1
                ca.resize(e_pos, <Py_ssize_t>e_pos_size)
                ...
                tmp = ca.clone(uint64_array_template, <Py_ssize_t>len(erase_pos_array), <bint>False)
                tmp_size = <uint64_t>0
                for j in range(<uint64_t>len(erase_pos_array)): # pyright: ignore[reportGeneralTypeIssues]
                    if erase_pos_array.data.as_ulonglongs[j] >= nsize: # pyright: ignore[reportGeneralTypeIssues]
                        tmp.data.as_ulonglongs[tmp_size] = erase_pos_array.data.as_ulonglongs[j] - nsize # pyright: ignore[reportGeneralTypeIssues]
                        tmp_size += 1
                erase_pos_array = tmp
            else:
                e_pos = ca.clone(uint64_array_template, <Py_ssize_t>0, <bint>False)
            # Decode/repair this chunk!
            rmes, recc, errata_pos = rs_correct_msg(data_array_view[chunk_size * i:chunk_size * (i + 1)], nsym_c, fcr, generator, e_pos, only_erasures) # pyright: ignore[reportGeneralTypeIssues]
            rmes_view, recc_view = rmes, recc # pyright: ignore[reportGeneralTypeIssues]
            dec_view[nmes * i:nmes * i + rmes_view.shape[0]] = rmes_view # pyright: ignore[reportGeneralTypeIssues]
            if i == total_chunks - 1:
                ca.resize(dec, <Py_ssize_t>nmes * i + rmes_view.shape[0])
            dec_full_view[nsize * i:nsize * i + rmes_view.shape[0]] = rmes_view # pyright: ignore[reportGeneralTypeIssues]
            if i == total_chunks - 1:
                ca.resize(dec_full, <Py_ssize_t>nsize * i + rmes_view.shape[0] + recc_view.shape[0])
                dec_full_view = dec_full # pyright: ignore[reportGeneralTypeIssues]
            dec_full_view[nsize * i + rmes_view.shape[0]:nsize * i + rmes_view.shape[0] + recc_view.shape[0]] = recc_view # pyright: ignore[reportGeneralTypeIssues] append corrected ecc just after corrected message. The two lines are equivalent to rmes + recc but here we don't need to concatenate both arrays first (and create a third one for nothing) before storing in the output array
            for j in range(<uint64_t>len(errata_pos)): # pyright: ignore[reportGeneralTypeIssues]
                errata_pos_all.data.as_ulonglongs[errata_pos_all_view_size] = errata_pos.data.as_ulonglongs[j] + i * nsize # pyright: ignore[reportGeneralTypeIssues]
                errata_pos_all_view_size += 1
        ca.resize(errata_pos_all, <Py_ssize_t>errata_pos_all_view_size)
        self.__restore_globals() #just incase someone is using the proceedural API at the same time.
        return dec, dec_full, errata_pos_all # pyright: ignore[reportGeneralTypeIssues]
    ...
    def check(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[pa.array[int], 'B']:
        ...
        cdef ca.array check = ca.clone(uint8_array_template, <Py_ssize_t>total_chunks, <bint>False) # TODO: could use a bitarray but this creates an external dependency and it's not compatible with cython https://pypi.org/project/bitarray/ and https://www.noveltech.dev/booleans-python-numpy/
        ...
        cdef uint64_t i
        for i in range(total_chunks): # pyright: ignore[reportGeneralTypeIssues] Split the long message in a chunk
            check.data.as_uchars[i] = <uint8_t>rs_check(data_array_view[chunk_size * i:chunk_size * (i + 1)], nsym_c, fcr, generator) # pyright: ignore[reportGeneralTypeIssues]
        return check # pyright: ignore[reportGeneralTypeIssues]
