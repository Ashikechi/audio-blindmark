...
::
    ...
    # Initialization
    >>> from creedsolo.reedsolo import RSCodec
...
def find_prime_polys(generator: int = 2, c_exp: int = 8, fast_primes: bool = False, single: bool = False) -> Annotated[array.array, 'Q']:
    ...
    # we're in GF(2)
    field_charac = (1 << c_exp) - 1 # pylint: disable = W0621
    field_charac_next = (1 << c_exp + 1) - 1
    ...
    if fast_primes:
        ...
        prim_candidates = array.array('Q', (x for x in prim_candidates if x > field_charac)) # filter out too small primes
    else:
        prim_candidates = array.array('Q', range(field_charac + 2, field_charac_next, 2)) # try each possible prime polynomial, but skip even numbers (because divisible by 2 so necessarily not irreducible)

    # Start of the main loop
    correct_primes = array.array('Q')
    for prim in prim_candidates: # try potential candidates primitive irreducible polys
        seen = array.array('Q', [0] * (field_charac + 1)) # memory variable to indicate if a value was already generated in the field (value at index x is set to 1) or not (set to 0 by default)
        ...
        if not conflict:
            if single:
                return array.array('Q', [prim])  # for API consistency, we always return an array, but here with a single value
...
def init_tables(prim: int = 0x11d, generator: int = 2, c_exp: int = 8) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q'], int]:
    ...
    # Init global tables
    global gf_exp, gf_log, field_charac # pylint: disable = W0603
    field_charac = (1 << c_exp) - 1
    gf_exp = array.array('Q', [0] * (field_charac * 2)) # anti-log (exponential) table. The first two elements will always be [GF256int(1), generator]
    gf_log = array.array('Q', [0] * (field_charac + 1)) # log table, log[0] is impossible and thus unused
...
# def gf_mult_noLUT_slow(x: int, y: int, prim: Optional[int] = None) -> int:
...
#     def cl_div(dividend: int, divisor: int):
...
def gf_mult_noLUT(x: int, y: int, prim: int = 0, field_charac_full: int = 256, carryless: bool = True) -> int:
...
def gf_poly_scale(p: Annotated[array.array, 'Q'], x: int) -> Annotated[array.array, 'Q']:
    '''Scale a galois field polynomial with a factor x (an integer)'''
    #return array.array('Q', (gf_mul(p[i], x) for i in range(len(p))))  # unoptimized one-liner
    out = array.array('Q', [0] * len(p))
...
def gf_poly_mul(p: Annotated[array.array, 'Q'], q: Annotated[array.array, 'Q']) -> Annotated[array.array, 'Q']:
    ...
    r = array.array('Q', [0] * (len(p) + len(q) - 1))
...
def gf_poly_mul_simple(p: Annotated[array.array, 'Q'], q: Annotated[array.array, 'Q']): # simple equivalent way of multiplying two polynomials without precomputation, but thus it's slower
...
def gf_poly_div(dividend: Annotated[array.array, 'Q'], divisor: Annotated[array.array, 'Q']) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q']]:
    ...
    msg_out = array.array('Q', dividend) # Copy the dividend list and pad with 0 where the ecc bytes will be computed
...
def gf_poly_square(poly: Annotated[array.array, 'Q']) -> Annotated[array.array, 'Q']:  # pragma: no cover
    ...
    length = len(poly)
    out = array.array('Q', [0] * (length * 2 - 1))
    for i in range(length - 1):
        p = poly[i]
        k = 2 * i
        if p != 0:
            #out[k] = gf_exp[(2 * gf_log[p]) % field_charac] # not necessary to modulo (2^r)-1 since gf_exp is duplicated up to 510.
            out[k] = gf_exp[gf_log[p] * 2]
        #else: # not necessary since the output is already initialized to an array of 0
            #out[k] = 0
    out[length * 2 - 2] = gf_exp[gf_log[poly[length - 1]] * 2]
    if out[0] == 0:
        out[0] = poly[1] * 2 - 1
    return out

def gf_poly_eval(poly: Annotated[array.array, 'Q'], x: int) -> int:
    '''Evaluates a polynomial in GF(2^p) given the value for x. This is based on Horner's scheme for maximum efficiency.'''
    y = poly[0]
    for i in range(1, len(poly)):
        y = gf_mul(y, x) ^ poly[i] # pylint: disable = W1114
    return y


################### REED-SOLOMON ENCODING ###################

def rs_generator_poly(nsym: int, fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']:
    '''Generate an irreducible generator polynomial (necessary to encode a message into Reed-Solomon)'''
    g = array.array('Q', [1])
    for i in range(nsym):
        g = gf_poly_mul(g, array.array('Q', [1, gf_pow(generator, i + fcr)]))
    return g

def rs_generator_poly_all(max_nsym: int, fcr: int = 0, generator: int = 2) -> list[Annotated[array.array, 'Q']]:
    ...
    #g_all[0] = g_all[1] = array.array('Q', [1])
    g_all = [array.array('Q', [1])] * max_nsym  # a list of list is potentially faster than using a dict, since it is pre-allocated and a list has less overhead than a dict
    for nsym in range(max_nsym):
        g_all[nsym] = rs_generator_poly(nsym, fcr, generator)
    return g_all

def rs_simple_encode_msg(msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, gen: Optional[Annotated[array.array, 'Q']] = None) -> Annotated[array.array, 'Q']:
    ...
    if (len(msg_in) + nsym) > field_charac:
        raise ValueError("Message is too long (%i when max is %i)" % (len(msg_in) + nsym, field_charac))
    if gen is None:
        gen = rs_generator_poly(nsym, fcr, generator)

    # Pad the message, then divide it by the irreducible generator polynomial
    _, remainder = gf_poly_div(msg_in + array.array('Q', [0] * (len(gen) - 1)), gen)
...
def rs_encode_msg(msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, gen: Optional[Annotated[array.array, 'Q']] = None) -> Annotated[array.array, 'Q']:
    ...
    msg_out = msg_in + array.array('Q', [0] * (len(gen) - 1)) # init msg_out with the values inside msg_in and pad with len(gen)-1 bytes (which is the number of ecc symbols).

    # Precompute the logarithm of every items in the generator
    lgen = array.array('Q', (gf_log[gen[i]] for i in range(len(gen))))
...
def inverted(msg: Annotated[array.array, 'Q']) -> Annotated[array.array, 'Q']:
    ...
    return array.array('Q', reversed(msg))

def rs_calc_syndromes(msg: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    return array.array('Q', [0] + [gf_poly_eval(msg, gf_pow(generator, i + fcr)) for i in range(nsym)])

def rs_correct_errata(msg_in: Annotated[array.array, 'Q'], synd: Annotated[array.array, 'Q'], err_pos: Annotated[array.array, 'Q'], fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']: # err_pos is a list of the positions of the errors/erasures/errata
    '''Forney algorithm, computes the values (error magnitude) to correct the input message.'''
    msg = array.array('Q', msg_in)
    ...
    coef_pos = array.array('Q', (len(msg) - 1 - p for p in err_pos)) # need to convert the positions to coefficients degrees for the errata locator algo to work (eg: instead of [0, 1, 2] it will become [len(msg)-1, len(msg)-2, len(msg) -3]), also can use a _bytearray() because length of message - and hence length of coef_pos - will always be constrained by the Galois Field.
    ...
    X = array.array('Q', [0] * len(coef_pos)) # will store the position of the errors
    for i in range(len(coef_pos)):
        l = field_charac - coef_pos[i]
        X[i] = gf_pow(generator, -l)

    # Forney algorithm: compute the magnitudes
    E = array.array('Q', [0] * len(msg)) # will store the values that need to be corrected (substracted) to the message containing errors. This is sometimes called the error magnitude polynomial.
    X_len = len(X)
    for i, Xi in enumerate(X):
        ...
        #err_loc_prime_tmp = array.array('Q')
...
def rs_find_error_locator(synd: Annotated[array.array, 'Q'], nsym: int, erase_loc: Optional[Annotated[array.array, 'Q']] = None, erase_count: int = 0) -> Annotated[array.array, 'Q']:
    ...
    if erase_loc: # if the erasure locator polynomial is supplied, we init with its value, so that we include erasures in the final locator polynomial
        err_loc = array.array('Q', erase_loc)
        old_loc = array.array('Q', erase_loc)
    else:
        err_loc = array.array('Q', [1]) # This is the main variable we want to fill, also called Sigma in other notations or more formally the errors/errata locator polynomial.
        old_loc = array.array('Q', [1]) # BM is an iterative algorithm, and we need the errata locator polynomial of the previous iteration in order to update other necessary variables.
    ...
    for i in range(nsym - erase_count): # generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
        ...
        delta = synd[K]
        for j in range(1, len(err_loc)):  # range 1:256 is important: if you use range 0:255, if the last byte of the ecc symbols is corrupted, it won't be correctable! You need to use the range 1,256 to include this last byte.
        ...
        old_loc = old_loc + array.array('Q', [0])
...
def rs_find_errata_locator(e_pos: Annotated[array.array, 'Q'], generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    e_loc = array.array('Q', [1]) # just to init because we will multiply, so it must be 1 so that the multiplication starts correctly without nulling any term
    ...
    for i in e_pos:
        e_loc = gf_poly_mul(e_loc, gf_poly_add(array.array('Q', [1]), array.array('Q', [gf_pow(generator, i), 0])))
    return e_loc

def rs_find_error_evaluator(synd: Annotated[array.array, 'Q'], err_loc: Annotated[array.array, 'Q'], nsym: int) -> Annotated[array.array, 'Q']:
    ...
    #_, remainder = gf_poly_div(gf_poly_mul(synd, err_loc), array.array('Q', [1] + [0] * (nsym + 1))) # first multiply syndromes * errata_locator, then do a polynomial division to truncate the polynomial to the required length
...
def rs_find_errors(err_loc: Annotated[array.array, 'Q'], nmess: int, generator: int = 2) -> Annotated[array.array, 'Q']:
...
def rs_forney_syndromes(synd: Annotated[array.array, 'Q'], pos: Annotated[array.array, 'Q'], nmess: int, generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    erase_pos_reversed = array.array('Q', (nmess - 1 - p for p in pos)) # prepare the coefficient degree positions (instead of the erasures positions)

    # Optimized method, all operations are inlined
    fsynd = array.array('Q', synd[1:])      # make a copy and trim the first coefficient which is always 0 by definition
...
def rs_correct_msg(msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, erase_pos: Optional[Annotated[array.array, 'Q']] = None, only_erasures: bool = False) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q'], Annotated[array.array, 'Q']]:
    ...
    msg_out = array.array('Q', msg_in)     # copy of message
...
#     # check if there's any error/erasure in the input codeword. If not (all syndromes coefficients are 0), then just return the codeword as-is.
#     if max(synd) == 0:
#         return msg_out[:-nsym], msg_out[-nsym:], array.array('Q')  # no errors
...
#     erase_pos_reversed = array.array('Q', (msg_out_len - 1 - eras for eras in erase_pos))
...
def rs_check(msg: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2) -> bool:
...
class RSCodec:
    ...
    def __init__(self, nsym: int = 10, nsize: int = 255, fcr: int = 0, prim: int = 0x11d, generator: int = 2, c_exp=8, single_gen: bool = True) -> None:
        ...
        if c_exp != 8 and prim == 0x11d:  # prim was not correctly defined, find one
            prim = find_prime_polys(generator, c_exp, True, True)[0]
        ...
        if single_gen:
            self.gen = {}
            self.gen[nsym] = rs_generator_poly(nsym, fcr, generator)
        else:
            self.gen = rs_generator_poly_all(nsize, fcr, generator)
    ...
    def __save_globals(self) -> None:
        self.__globals_temp = (gf_log, gf_exp, field_charac) # pylint: disable = W0201

    def __set_globals(self) -> None:
        global gf_log, gf_exp, field_charac # pylint: disable = W0603
        gf_log, gf_exp, field_charac = self.gf_log, self.gf_exp, self.field_charac

    def __restore_globals(self) -> None:
        global gf_log, gf_exp, field_charac # pylint: disable = W0603
        gf_log, gf_exp, field_charac = self.__globals_temp
    ...
    def encode(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[array.array, 'Q']:
        ...
        data_array = array.array('Q', data)

        # Calculate chunk size and total number of chunks for looping
        chunk_size = int(nsize - nsym)
        total_chunks = (len(data_array) - 1) // chunk_size + 1 # need to convert to floats first to get an accurate floating division, or else we assume implicit conversion and it will cause an error on Python 2

        # Preallocate output array
        enc = array.array('Q', [0] * nsize * total_chunks) # pre-allocate array and we will overwrite data in it, much faster than extending
        # Chunking loop
        for i in range(total_chunks):
            # Encode this chunk and update a slice of the output bytearray, much more efficient than extending an array constantly
            enc[nsize * i:nsize * (i + 1)] = rs_encode_msg(data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator, gen)
    ...
    def decode(self, data: Iterable[int], nsym: Optional[int] = None, erase_pos: Optional[Iterable[int]] = None, only_erasures: bool = False) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q'], Annotated[array.array, 'Q']]:
        ...
        data_array = array.array('Q', data)
        erase_pos_array = array.array('Q', erase_pos) if erase_pos is not None else None
        ...
        dec = array.array('Q', [0] * (total_chunks * nmes)) # pre-allocate array and we will overwrite data in it, much faster than extending
        dec_full = array.array('Q', [0] * (total_chunks * nsize))
        errata_pos_all = array.array('Q')
        # Chunking loop
        for i in range(total_chunks):  # Split the long message in a chunk
            # Extract the erasures for this chunk
            if erase_pos_array is not None:
                # First extract the erasures for this chunk (all erasures below the maximum chunk length)
                e_pos = array.array('Q', (x for x in erase_pos_array if x < nsize))
                ...
                erase_pos_array = array.array('Q', (x - nsize for x in erase_pos_array if x >= nsize))
            else:
                e_pos = array.array('Q')
            # Decode/repair this chunk!
            rmes, recc, errata_pos = rs_correct_msg(data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator, e_pos, only_erasures)
            dec[nmes * i:nmes * (i + 1)] = rmes
            dec_full[nsize * i:nsize * (i + 1)] = rmes
            dec_full[nsize * i + nmes:nsize * (i + 1) + nmes] = recc # append corrected ecc just after corrected message. The two lines are equivalent to rmes + recc but here we don't need to concatenate both arrays first (and create a third one for nothing) before storing in the output array
            errata_pos_all.extend((each + nsize * i for each in errata_pos))
    ...
    def check(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[array.array, 'B']:
        ...
        check = array.array('B', [False] * total_chunks) # TODO: could use a bitarray but this creates an external dependency and it's not compatible with cython https://pypi.org/project/bitarray/ and https://www.noveltech.dev/booleans-python-numpy/
        # Chunking loop
        for i in range(0, total_chunks): # Split the long message in a chunk
            check[i] = rs_check(data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator)
    ...
    def maxerrata(self, nsym: Optional[int] = None, errors: Optional[int] = None, erasures: Optional[int] = None, verbose: bool = False) -> tuple[int, int]:
        ...
        maxerrors = nsym // 2  # always floor the number, we can't correct half a symbol, it's all or nothing
        maxerasures = nsym
