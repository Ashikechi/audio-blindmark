...
import array
from typing import Annotated, Generator, Iterable, Optional, Sequence, Union
...
def gf_poly_mul(gf: GaloisField, p: Annotated[array.array, 'Q'], q: Annotated[array.array, 'Q']) -> Annotated[array.array, 'Q']:
    ...
    lp = [gf.gf_log[p[i]] for i in range(len(p))]
    ...
    for j in range(len(q)):
        ...
        if qj != 0:  # log(0) is undefined, we need to check that
            lq = gf.gf_log[qj]  # Optimization: precache the logarithm of the current coefficient of q
            for i in range(len(p)):
                if p[i] != 0:  # log(0) is undefined, need to check that...
                    r[i + j] ^= gf.gf_exp[lp[i] + lq]  # equivalent to: r[i + j] = gf_add(r[i + j], gf_mul(p[i], q[j]))
    return r
...
#     for i in range(len(p)):
#         for j in range(len(q)):
#             r[i + j] ^= gf.gf_mul(p[i], q[j])  # equivalent to: r[i + j] = gf_add(r[i + j], gf_mul(p[i], q[j])) -- you can see it's your usual polynomial multiplication
#     return r
        ...
        if coef != 0:  # log(0) is undefined, so we need to avoid that case explicitly (and it's also a good optimization). In fact if you remove it, it should still work because gf_mul() will take care of the condition. But it's still a good practice to put the condition here.
            for j in range(1, divisor_len):  # in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient
                if divisor[j] != 0:  # log(0) is undefined
                    msg_out[i + j] ^= gf.gf_mul(divisor[j], coef)  # equivalent to the more mathematically correct (but xoring directly is faster): msg_out[i + j] += -divisor[j] * coef
...
def gf_poly_square(gf: GaloisField, poly: Annotated[array.array, 'Q']) -> Annotated[array.array, 'Q']: # pragma: no cover
    ...
    for i in range(length - 1):
        ...
        k = 2 * i
        if p != 0:
            #out[k] = gf_exp[(2 * gf.gf_log[p]) % field_charac]  # not necessary to modulo (2^r)-1 since gf_exp is duplicated up to 510.
            out[k] = gf.gf_exp[gf.gf_log[p] * 2]
        #else: # not necessary since the output is already initialized to an array of 0
            #out[k] = 0
    out[length * 2 - 2] = gf.gf_exp[gf.gf_log[poly[length - 1]] * 2]
    if out[0] == 0:
        out[0] = poly[1] * 2 - 1
    return out
...
def rs_generator_poly(gf: GaloisField, nsym: int, fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    g = array.array('Q', [1])
    for i in range(nsym):
        g = gf_poly_mul(gf, g, array.array('Q', [1, gf.gf_pow(generator, i + fcr)]))
    return g
...
def rs_generator_poly_all(gf: GaloisField, max_nsym: int, fcr: int = 0, generator: int = 2) -> list[Annotated[array.array, 'Q']]:
    ...
    for nsym in range(max_nsym):
        g_all[nsym] = rs_generator_poly(gf, nsym, fcr, generator)
    return g_all
...
def rs_simple_encode_msg(gf: GaloisField, msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, gen: Optional[Annotated[array.array, 'Q']] = None) -> Annotated[array.array, 'Q']:
    ...
    if len(msg_in) + nsym > gf.field_charac:
        raise ValueError(f'Message is too long ({len(msg_in) + nsym} when max is {gf.field_charac})')
    if gen is None:
        gen = rs_generator_poly(gf, nsym, fcr, generator)

    # Pad the message, then divide it by the irreducible generator polynomial
    _, remainder = gf_poly_div(gf, msg_in + array.array('Q', [0] * (len(gen) - 1)), gen)
...
def rs_encode_msg(gf: GaloisField, msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, gen: Optional[Annotated[array.array, 'Q']] = None) -> Annotated[array.array, 'Q']:
    ...
    lgen = array.array('Q', (gf.gf_log[gen[i]] for i in range(len(gen))))
    ...
    for i in range(msg_in_len):
        ...
        if coef != 0:  # log(0) is undefined, so we need to manually check for this case. There's no need to check the divisor here because we know it can't be 0 since we generated it.
            lcoef = gf.gf_log[coef]  # precaching
            ...
            for j in range(1, gen_len):  # in synthetic division, we always skip the first coefficient of the divisior, because it's only used to normalize the dividend coefficient (which is here useless since the divisor, the generator polynomial, is always monic)
                ...
                msg_out[i + j] ^= gf.gf_exp[lcoef + lgen[j]]  # optimization, equivalent to gf_mul(gen[j], msg_out[i]) and we just substract it to msg_out[i+j] (but since we are in GF256, it's equivalent to an addition and to an XOR). In other words, this is simply a "multiply-accumulate operation"
...
def rs_calc_syndromes(gf: GaloisField, msg: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    return array.array('Q', [0] + [gf_poly_eval(gf, msg, gf.gf_pow(generator, i + fcr)) for i in range(nsym)])
...
def rs_correct_errata(gf: GaloisField, msg_in: Annotated[array.array, 'Q'], synd: Annotated[array.array, 'Q'], err_pos: Annotated[array.array, 'Q'], fcr: int = 0, generator: int = 2) -> Annotated[array.array, 'Q']: # err_pos is a list of the positions of the errors/erasures/errata
    ...
    err_loc = rs_find_errata_locator(gf, coef_pos, generator)
    # calculate errata evaluator polynomial (often called Omega or Gamma in academic papers)
    err_eval = inverted(rs_find_error_evaluator(gf, inverted(synd), err_loc, len(err_loc) - 1))
    ...
    for i in range(len(coef_pos)):
        l = gf.field_charac - coef_pos[i]
        x[i] = gf.gf_pow(generator, -l)
    ...
    for i, x_i in enumerate(x):
        ...
        #    err_loc_prime = gf.gf_mul(err_loc_prime, coef)
        ...
        for j in range(x_len):
            if j != i:
                err_loc_prime = gf.gf_mul(err_loc_prime, gf_sub(1, gf.gf_mul(x_i_inv, x[j])))
        ...
        y = gf.gf_mul(gf.gf_pow(x_i, 1 - fcr), y)  # adjust to fcr parameter
...
def rs_find_error_locator(gf: GaloisField, synd: Annotated[array.array, 'Q'], nsym: int, erase_loc: Optional[Annotated[array.array, 'Q']] = None, erase_count: int = 0) -> Annotated[array.array, 'Q']:
    ...
    for i in range(nsym - erase_count):  # generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
        ...
        for j in range(1, len(err_loc)):  # range 1:256 is important: if you use range 0:255, if the last byte of the ecc symbols is corrupted, it won't be correctable! You need to use the range 1,256 to include this last byte.
            delta ^= gf.gf_mul(err_loc[-(j + 1)], synd[k - j])  # delta is also called discrepancy. Here we do a partial polynomial multiplication (ie, we compute the polynomial multiplication only for the term of degree K). Should be equivalent to brownanrs.polynomial.mul_at().
...
def rs_find_errata_locator(gf: GaloisField, e_pos: Annotated[array.array, 'Q'], generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    for i in e_pos:
        e_loc = gf_poly_mul(gf, e_loc, gf_poly_add(array.array('Q', [1]), array.array('Q', [gf.gf_pow(generator, i), 0])))
    return e_loc
...
def rs_find_error_evaluator(gf: GaloisField, synd: Annotated[array.array, 'Q'], err_loc: Annotated[array.array, 'Q'], nsym: int) -> Annotated[array.array, 'Q']:
    ...
    remainder = gf_poly_mul(gf, synd, err_loc)  # first multiply the syndromes with the errata locator polynomial
...
def rs_forney_syndromes(gf: GaloisField, synd: Annotated[array.array, 'Q'], pos: Annotated[array.array, 'Q'], nmess: int, generator: int = 2) -> Annotated[array.array, 'Q']:
    ...
    for i in range(len(pos)):
        x = gf.gf_pow(generator, erase_pos_reversed[i])
        for j in range(len(fsynd) - 1):
            fsynd[j] = gf.gf_mul(fsynd[j], x) ^ fsynd[j + 1]
...
def rs_correct_msg(gf: GaloisField, msg_in: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2, erase_pos: Optional[Annotated[array.array, 'Q']] = None, only_erasures: bool = False) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q'], Annotated[array.array, 'Q']]:
    ...
    if len(msg_in) > gf.field_charac:
        ...
        raise ValueError(f'Message is too long ({len(msg_in)} when max is {gf.field_charac})')
    ...
    if only_erasures:
        ...
        err_loc = rs_find_error_locator(gf, fsynd, nsym, erase_count = len(erase_pos))
    ...
    msg_out = rs_correct_errata(gf, msg_out, synd, erase_pos + err_pos, fcr, generator)  # note that we here use the original syndrome, not the forney syndrome (because we will correct both errors and erasures, so we need the full syndrome)
...
def rs_check(gf: GaloisField, msg: Annotated[array.array, 'Q'], nsym: int, fcr: int = 0, generator: int = 2) -> bool:
    ...
    return max(rs_calc_syndromes(gf, msg, nsym, fcr, generator)) == 0
...
class RSCodec:
    ...
    def __init__(self, nsym: int = 10, nsize: int = 255, fcr: int = 0, prim: int = 0x11d, generator: int = 2, c_exp=8, single_gen: bool = True) -> None:
        ...
        self.gen: Union[list[Annotated[array.array, 'Q']], dict[int, Annotated[array.array, 'Q']]]
        if single_gen:
            self.gen = {}
            self.gen[nsym] = rs_generator_poly(self.gf, nsym, fcr, generator)
        else:
            self.gen = rs_generator_poly_all(self.gf, nsize, fcr, generator)
    ...
    def encode(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[array.array, 'Q']:
        ...
        for i in range(total_chunks):
            ...
            enc[nsize * i:nsize * (i + 1)] = rs_encode_msg(self.gf, data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator, gen)
    ...
    def decode(self, data: Iterable[int], nsym: Optional[int] = None, erase_pos: Optional[Iterable[int]] = None, only_erasures: bool = False) -> tuple[Annotated[array.array, 'Q'], Annotated[array.array, 'Q'], Annotated[array.array, 'Q']]:
        ...
        for i in range(total_chunks):  # Split the long message in a chunk
            ...
            rmes, recc, errata_pos = rs_correct_msg(self.gf, data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator, e_pos, only_erasures)
    ...
    def check(self, data: Iterable[int], nsym: Optional[int] = None) -> Annotated[array.array, 'B']:
        ...
        for i in range(0, total_chunks):  # Split the long message in a chunk
            check[i] = rs_check(self.gf, data_array[chunk_size * i:chunk_size * (i + 1)], nsym, fcr, generator)
        return check
